...
- Run all GitHub commands only through `scripts/gh-readonly.sh`.
- Resolve `scripts/gh-readonly.sh` relative to this skill directory.
- Verify CLI auth first with `scripts/gh-readonly.sh auth status`. If not authenticated, ask the user to run `gh auth login`.
- Enforce strict read-only mode at all times.
- Never bypass `scripts/gh-readonly.sh` by calling `gh` directly.
...
1. Prepare review context.
   - Confirm identity and auth: `scripts/gh-readonly.sh auth status`, `scripts/gh-readonly.sh api user`.
   - Resolve repository owner/name from the current repo or pass `-R <OWNER>/<REPO>`.
2. Resolve the target PR.
   - Use `scripts/gh-readonly.sh pr view <PR_NUMBER> [--json <fields>]` when PR number is known.
   - Otherwise shortlist with `scripts/gh-readonly.sh pr list [flags]` and pick the target PR.
3. Sync local repository to the latest PR branch code.
...
4. Gather full PR evidence before judging.
   - Metadata: `scripts/gh-readonly.sh pr view <PR_NUMBER> [--json <fields>]`
   - Diff: `scripts/gh-readonly.sh pr diff <PR_NUMBER> [--patch|--name-only]`
   - Changed files: `scripts/gh-readonly.sh api repos/<OWNER>/<REPO>/pulls/<PR_NUMBER>/files --paginate`
   - Reviews: `scripts/gh-readonly.sh api repos/<OWNER>/<REPO>/pulls/<PR_NUMBER>/reviews --paginate`
   - Checks: `scripts/gh-readonly.sh pr checks <PR_NUMBER> [--json <fields>]`
   - Comments:
     - `scripts/gh-readonly.sh pr view <PR_NUMBER> --comments`
     - `scripts/gh-readonly.sh api repos/<OWNER>/<REPO>/issues/<PR_NUMBER>/comments --paginate`
     - `scripts/gh-readonly.sh api repos/<OWNER>/<REPO>/pulls/<PR_NUMBER>/comments --paginate`
5. Inspect changed backend code deeply.
   ...
   - Use `scripts/gh-readonly.sh api repos/<OWNER>/<REPO>/contents/<PATH>?ref=<REF>` when exact remote content is needed (content is usually base64 in `.content`).
6. Apply review checklist with risk-first ordering.
...
7. Produce actionable review output.
